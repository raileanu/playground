...
class RolloutStorage(object):
    def __init__(self, num_steps, nagents, num_processes, obs_shape, action_space, state_size):
        ...
        self.rewards = torch.zeros(num_steps, nagents, num_processes, 1)
    ...
    def compute_returns(self, next_value, use_gae, gamma, tau, nagents):
        for i in range(nagents):
            if use_gae:
                self.value_preds[-1][i] = next_value[i]
                gae = 0
                for step in reversed(range(self.rewards[i].size(0))):
                    delta = self.rewards[step][i] + gamma * self.value_preds[step + 1][i] * self.masks[step + 1][i] - self.value_preds[step][i]
                    gae = delta + gamma * tau * self.masks[step + 1][i] * gae
                    self.returns[step][i] = gae + self.value_preds[step][i]
            else:
                self.returns[-1][i] = next_value[i]
                for step in reversed(range(self.rewards[i].size(0))):
    ...
    def feed_forward_generator(self, advantages, num_mini_batch, args, i):
        ...
        for indices in sampler:
            ...
            return_batch = self.returns[:-1].select(1, i).contiguous().view((args.num_steps*args.num_processes), 1)[indices]
    ...
    # TODO: need to fix the recurrent generator as well - similar to above fix
    def recurrent_generator(self, advantages, num_mini_batch, args, i):
        num_processes = self.rewards.size(2)
